...
export default function EditProductPage() {
  ...
  const fetchProduct = async (id) => {
    try {
      ...
      setFormData({
        type: product.type || 'whisky',
        name: product.name || '',
        description: product.description || '',
        level: product.level || 'beginner',
        chemical_note: product.chemical_note || '',
        cask_type: product.cask_type || '',
        age: product.age || '',
        polishing_rate: product.polishing_rate || '',
        rice_type: product.rice_type || '',
        serving_temp_cold: product.serving_temp_range?.cold || '',
        serving_temp_room: product.serving_temp_range?.room || '',
        serving_temp_warm: product.serving_temp_range?.warm || '',
        ibu: product.ibu || '',
        hop_type: product.hop_type || '',
        fermentation_method: product.fermentation_method || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || '제품 수정에 실패했습니다.');
        setLoading(false);
        return;
      }
